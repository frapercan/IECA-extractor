...
if __name__ == "__main__":
    ...
    for directorio_datos in directorios_datos:
        if '2' in directorio_datos and listdir(join(directorio, directorio_datos)):
            ...
            cuadros_de_datos_fusionados = pd.concat(
                [pd.read_csv(join(directorio, directorio_datos, fichero), dtype='string', index_col=None) for fichero in
                 ficheros_fusionar])
            try:
                cuadros_de_datos_fusionados.to_csv(join(directorio, directorio_datos, directorio_datos) + '.csv',
                                                   index=False)
